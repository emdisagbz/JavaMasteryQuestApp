...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function createParticle() {
    ...
    const vx = -Math.random() * (maxSpeed - minSpeed) - minSpeed; // Move towards left
...
document.getElementById('startButton').addEventListener('click', function() {
    ...
    particles.forEach(particle => {
        particle.style.animation = 'moveToLeft 2s infinite linear';
    });
});
...
window.onload = function() {
	...
	document.getElementById('section').textContent = localStorage.getItem('section');
};
